...
| **RF-039** | Exposición de métricas del API Gateway | El sistema debe exponer un endpoint `/metrics` en el API Gateway para entregar métricas en formato compatible con Prometheus. | Alta |
| **RF-040** | Registro de métricas por solicitud HTTP | El API Gateway debe registrar por cada solicitud HTTP el método, ruta, código de estado, cantidad de peticiones y duración acumulada. | Alta |
| **RF-041** | Generación de logs estructurados | El API Gateway debe generar logs estructurados en formato JSON para facilitar la búsqueda de eventos, errores HTTP, rutas consultadas y tiempos de respuesta. | Alta |
| **RF-042** | Centralización de logs con Stack ELK | El sistema debe recolectar logs de contenedores Docker mediante Filebeat, procesarlos con Logstash, almacenarlos en Elasticsearch y visualizarlos en Kibana. | Alta |
| **RF-043** | Consulta de logs en Kibana | El equipo técnico debe poder consultar, filtrar y analizar logs del proyecto mediante índices de Elasticsearch visibles desde Kibana. | Alta |
| **RF-044** | Pruebas de carga con Locust | El sistema debe permitir ejecutar pruebas de carga HTTP contra endpoints reales del API Gateway utilizando Locust. | Alta |
| **RF-045** | Generación de reporte HTML de carga | Locust debe generar un reporte HTML con estadísticas de solicitudes, tiempos de respuesta, errores y comportamiento general durante la prueba. | Alta |
| **RF-046** | Configuración de stack de métricas | El sistema debe incluir Prometheus y Grafana para recolectar, consultar y visualizar métricas operativas del API Gateway y del entorno de contenedores. | Alta |
| **RF-047** | Recolección de métricas de contenedores | El sistema debe recolectar métricas de uso de contenedores mediante cAdvisor, incluyendo consumo de CPU, memoria y estado operativo. | Media |
| **RF-048** | Recolección de métricas del host | El sistema debe recolectar métricas del host mediante node-exporter para observar recursos base del entorno de ejecución. | Media |
| **RF-049** | Smoke tests de endpoints críticos | El sistema debe incluir pruebas rápidas de verificación para validar que endpoints críticos respondan después de un despliegue. | Alta |
| **RF-050** | Documentación de evidencias de Fase 3 | El sistema debe contar con documentación técnica que incluya capturas y evidencias de Locust, ELK, métricas y validaciones realizadas. | Alta |
...
| **RNF-028** | Observabilidad - Métricas | El sistema debe exponer métricas operativas para facilitar monitoreo y diagnóstico técnico. | Endpoint `/metrics` disponible en el API Gateway y scrapeado por Prometheus cada 15 segundos. |
| **RNF-029** | Observabilidad - Logs centralizados | Los logs generados por servicios deben poder centralizarse para análisis técnico. | Logs de contenedores recolectados por Filebeat, procesados por Logstash y almacenados en índices `quetxal-tv-logs-*` en Elasticsearch. |
| **RNF-030** | Trazabilidad de solicitudes HTTP | Cada solicitud recibida por el API Gateway debe dejar evidencia consultable. | Log JSON por request con `timestamp`, `service`, `event`, `method`, `path`, `status_code`, `duration_ms`, `user_agent` e `ip`. |
| **RNF-031** | Pruebas de carga ligera | La plataforma debe poder evaluarse con tráfico concurrente controlado. | Ejecución de Locust con 25 usuarios, razón de arranque de 5 usuarios/segundo y duración mínima de 2 minutos en ambiente local o configurado por `LOCUST_HOST`. |
| **RNF-032** | Evidencia de rendimiento | Las pruebas de carga deben generar evidencia verificable. | Reporte HTML `locust_result.html` con requests, fallos, latencias, percentiles y requests por segundo. |
| **RNF-033** | Visualización operacional | Las métricas y logs deben poder revisarse mediante herramientas visuales. | Grafana disponible en puerto 3001 para métricas y Kibana disponible en puerto 5601 para logs. |
| **RNF-034** | Bajo acoplamiento de observabilidad | Los microservicios no deben depender directamente de Elasticsearch ni Grafana para funcionar. | Recolección de logs por `stdout/stderr` de contenedores y lectura externa por Filebeat sin modificar la lógica de negocio de cada servicio. |
| **RNF-035** | Portabilidad del stack de observabilidad | La observabilidad debe poder levantarse junto al entorno local sin modificar el compose principal. | Stack definido en `observability/docker-compose.observability.yml` y combinable con `docker-compose.local.yml`. |
| **RNF-036** | Smoke testing post-despliegue | Después de desplegar, el sistema debe validar disponibilidad básica antes de considerar exitoso el pipeline. | Suite `tests/smoke/smoke_test.py` ejecutada contra `BASE_URL` y con fallo del pipeline si una ruta crítica no responde con estado esperado. |
| **RNF-037** | Compatibilidad con contenedores | La solución de logs y métricas debe ser compatible con servicios ejecutados en Docker. | Filebeat lee `/var/lib/docker/containers/*/*.log`, cAdvisor lee métricas de Docker y Prometheus scrapea targets definidos por nombre de servicio. |
| **RNF-038** | Seguridad de configuración | La configuración sensible no debe agregarse directamente al código ni a la documentación. | Uso de variables de entorno, archivos `.env` locales ignorados por Git y GitHub Secrets para CI/CD. |
| **RNF-039** | Diagnóstico de errores | Los errores HTTP deben ser diferenciables por severidad para facilitar análisis. | Logs con nivel `error` para respuestas 5xx, `warn` para 4xx e `info` para respuestas exitosas. |
| **RNF-040** | Impacto controlado de observabilidad | La instrumentación de métricas y logs no debe interrumpir el flujo de negocio. | Middleware de observabilidad registra eventos al finalizar la respuesta HTTP usando `res.on('finish')`. |
